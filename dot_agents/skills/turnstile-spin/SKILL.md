...
Work through the authorised setup without pausing between routine inspection and local edits. Complete preparation before requesting approval. Before widget creation or Worker deployment, confirm the exact account, domains, resource names, and action unless the user already explicitly authorised those values. Preserve required provider approvals.

1. **Begin preparation.** Briefly state the scope, then inspect auth and the codebase in the same turn. A setup request does not need a second "Proceed?" before inspection.
...
3. **Auth + scope probe (FIRST irreversible action).** Run `scripts/auth-probe.sh`. Branch on `status`:
   ...
   - `missing_token`, `missing_scope`, or `missing_workers_scope`: ask the user to provision the required account-scoped token through a secure channel. Use the current Cloudflare permission documentation to identify required scopes; do not request the token in chat. Environment/file options:
     ...
     Keep credentials out of chat. Prefer the active harness's secret store or the environment/file methods above.
     Continue independent domain and codebase inspection while credentials are unavailable. Once auth is established, re-run `auth-probe.sh` and resume at the first incomplete step.
...
12. **Persist skill.** Reuse the installed skill. Create a project-local copy only when requested, using the actual harness directory; do not add a persistence question to every setup.
